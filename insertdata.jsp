...
<%
					...
					try {
						...
						while (params.hasMoreElements()) 
						{
							...
							if(paramname.equalsIgnoreCase("group"))
							{
						...
						//image = image.replace(".", "_b.");
			 			...
			 			String query1="select * from user  where username='"+uname+"' and type='"+grp+"'"; 
					...
					if ( rs1.next() )
					   ...
					   {
							PreparedStatement ps=connection.prepareStatement("insert into user(type,username,password,email,mobile,dob,gender,address,pincode,status,image) values(?,?,?,?,?,?,?,?,?,?,?)");
						ps.setString(1,grp);
